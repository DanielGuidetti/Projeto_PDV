...
const renderProductsTable = (searchTerm = '') => {
    ...
    if (searchTerm) {
        ...
        filtered = filtered.filter(p => normalizeName(p.nome).includes(term) || String(p.PLU).includes(term) || (p.codigo_referencia && String(p.codigo_referencia).toLowerCase().includes(term)));
    }
...
document.getElementById('product-form').addEventListener('submit', async (e) => {
    ...
    const codigo_referencia = document.getElementById('prod-ref-code').value.trim() || null;
    ...
    const newProduct = { PLU, codigo_referencia, nome, preco, custo, estoque, pesavel, controlar_estoque, permitir_estoque_negativo, user_id: state.currentUser.id }; // id gerado automaticamente pelo banco
...
window.editProduct = (id) => {
    ...
    document.getElementById('prod-ref-code').value = product.codigo_referencia || '';
...
const renderPosCatalog = (searchTerm = '') => {
    ...
    const filtered = state.products.filter(p => {
        return p.nome.toLowerCase().includes(term) || p.PLU.includes(term) || (p.codigo_referencia && String(p.codigo_referencia).toLowerCase().includes(term));
    });
...
document.getElementById('pos-search').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
        ...
        // Busca produto pelo PLU exato (prioridade para leitor) ou código de referência
        const product = state.products.find(p => p.PLU === val || p.codigo_referencia === val);
...
const addToCart = (product, requestedQty = 1, fromScaleConfirm = false) => {
    // Intercept if it is a weighable product and we are not confirming from the scale modal
...
window.openImportExcelModal = () => {
    document.getElementById('import-excel-modal').classList.add('active');
};

document.querySelector('#import-excel-modal .close-modal').addEventListener('click', () => {
    document.getElementById('import-excel-modal').classList.remove('active');
});

document.getElementById('close-import-review').addEventListener('click', () => {
    document.getElementById('import-review-modal').classList.remove('active');
    state.importingProducts = [];
    document.getElementById('excel-file-input').value = ""; // reset
});

window.downloadExcelTemplate = () => {
    const ws_data = [
        ["Código de barras", "Quantidade", "Nome do produto", "Custo do produto", "Preço de venda", "Cód de referência", "É pesável?", "Controla estoque?"]
    ];
        ...
        try {
            ...
            const productsToImport = rows.map(row => {
                ...
                return {
                    ...
                    cost: parseFloat(row[3]) || null,
                    price: parseFloat(row[4]) || 0,
                    refCode: row[5] ? row[5].toString().trim() : '',
                    pesavel: isSim(row[6]),
                    controlStock: isSim(row[7])
                };
            }).filter(p => p.name && p.price > 0 && p.plu);
            ...
            state.importingProducts = productsToImport;
            state.currentImportIndex = 0;
            document.getElementById('import-excel-modal').classList.remove('active');
            document.getElementById('import-review-modal').classList.add('active');
            renderImportReview();

        } catch (error) {
            console.error(error);
            showToast("Erro ao ler a planilha.", "error");
            document.getElementById('excel-file-input').value = "";
        }
    };
...
function renderImportReview() {
    if (state.currentImportIndex >= state.importingProducts.length) {
        document.getElementById('import-review-modal').classList.remove('active');
        showToast("Revisão de importação concluída!", "success");
        document.getElementById('excel-file-input').value = "";
        if (typeof renderProductsTable === 'function') renderProductsTable();
        return;
    }

    const prod = state.importingProducts[state.currentImportIndex];
    document.getElementById('import-progress-text').innerText = `${state.currentImportIndex + 1}/${state.importingProducts.length}`;
    
    document.getElementById('import-prod-code').value = prod.plu;
    document.getElementById('import-prod-ref-code').value = prod.refCode || '';
    document.getElementById('import-prod-name').value = prod.name;
    document.getElementById('import-prod-cost').value = prod.cost || '';
    document.getElementById('import-prod-price').value = prod.price;
    document.getElementById('import-prod-control-stock').checked = prod.controlStock;
    document.getElementById('import-prod-stock').value = prod.stock;
    document.getElementById('import-prod-pesavel').checked = prod.pesavel;
}

document.getElementById('import-btn-prev').addEventListener('click', () => {
    if (state.currentImportIndex > 0) {
        state.currentImportIndex--;
        renderImportReview();
    }
});

document.getElementById('import-btn-skip').addEventListener('click', () => {
    state.currentImportIndex++;
    renderImportReview();
});

document.getElementById('import-btn-accept-all').addEventListener('click', async () => {
    const btn = document.getElementById('import-btn-accept-all');
    btn.disabled = true;
    ...
    const remaining = state.importingProducts.slice(state.currentImportIndex);
    
    for (const prod of remaining) {
        let finalPLU = prod.plu;
        if (!finalPLU) {
            let i = 1;
            const existingPLUs = new Set(state.products.map(p => p.PLU));
            while (existingPLUs.has(String(i))) {
                i++;
            }
            finalPLU = String(i);
        ...
        const existing = state.products.find(p => p.PLU === finalPLU);
        if (existing) {
            skipCount++;
            continue;
        }
        ...
        try {
            const newProd = {
                "PLU": finalPLU,
                nome: prod.name,
                custo: prod.cost || null,
                preco: prod.price,
                estoque: prod.stock || 0,
                pesavel: prod.pesavel,
                controlar_estoque: prod.controlStock,
                permitir_estoque_negativo: false,
                codigo_referencia: prod.refCode || null,
                user_id: state.currentUser.id
            };

            const { data, error } = await _supabase.from('produtos').insert([newProd]).select();
            if (error) throw error;

            if (prod.stock > 0 && prod.controlStock) {
            ...
            console.error("Erro importando lote:", prod.plu, err);
    ...
    btn.disabled = false;
    btn.innerHTML = '<i class="fas fa-check-double"></i> Aceitar Todos';
    
    if (skipCount > 0) {
        showToast(`Importação finalizada! ${successCount} salvos, ${skipCount} falhas. Motivo: ${lastErrorMsg ? lastErrorMsg : 'Duplicado/Erro'}`, "warning");
    } else {
        showToast(`Importação finalizada! ${successCount} produtos cadastrados com sucesso.`, "success");
    }
    
    state.currentImportIndex = state.importingProducts.length;
    renderImportReview();
});

document.getElementById('import-review-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    
    const code = document.getElementById('import-prod-code').value.trim();
    const refCode = document.getElementById('import-prod-ref-code').value.trim();
    const name = document.getElementById('import-prod-name').value.trim();
    const cost = document.getElementById('import-prod-cost').value ? parseFloat(document.getElementById('import-prod-cost').value) : null;
    const price = parseFloat(document.getElementById('import-prod-price').value);
    const controlStock = document.getElementById('import-prod-control-stock').checked;
    const stock = parseFloat(document.getElementById('import-prod-stock').value) || 0;
    const pesavel = document.getElementById('import-prod-pesavel').checked;
    ...
    const existing = state.products.find(p => p.PLU === finalPLU);
    if (existing) {
        showToast(`O código de barras ${code} já está cadastrado no produto "${existing.nome}".`, "error");
    ...
    try {
        const newProd = {
            ...
            estoque: stock,
            pesavel: pesavel,
            controlar_estoque: controlStock,
            permitir_estoque_negativo: false,
            codigo_referencia: refCode || null,
            user_id: state.currentUser.id
        };
        ...
        // Registrar entrada de estoque se for maior que zero
        if (stock > 0 && controlStock) {
            const { data: movData, error: movError } = await _supabase.from('movimentacoes').insert([{
                produto_id: data[0].id,
                tipo: 'ENTRADA',
                quantidade: stock,
        ...
        console.error(err);
        showToast("Erro: " + (err.message || "Falha ao cadastrar produto"), "error");
    }
});
